...
const useSetup = (dimensions: { width: number; height: number }) => {
  const circles = useMemo(() => {
    ...
    for (let i = 0; i < numCircles; i++) {
      const randomOffsetAngle = 20 || random(-angle * 0.4, angle * 0.4);
      const randomOffsetDistance = 20 || random(0, circleDiameter);
...
const useDraw = (
  circles: { x: Animated.Value<number>; y: Animated.Value<number> }[]
) => {
  const nativeCode = useMemo(() => {
    ...
    for (let i = 0; i < circles.length; i++) {
      ...
      nativeCode.push(set(circle.x, add(circle.x, multiply(circle.x, -0.05))));
...
export const useGravityAnimation = (dimensions: any) => {
